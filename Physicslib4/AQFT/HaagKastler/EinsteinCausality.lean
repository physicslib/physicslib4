...
the canonical quasilocal algebra `quasilocal`, the images of the local
...
theorem einstein_causality
    ...
    Commute (π (N.quasilocal.ι hB₁ a)) (π (N.quasilocal.ι hB₂ b)) :=
...
on `H`. -/
theorem exists_gns_einstein_causality (ω : State N.quasilocal.carrier) :
    ∃ (H : Type u)
      ...
      (π : N.quasilocal.carrier →⋆ₐ[ℂ] (H →L[ℂ] H)) (Ω : H),
        ...
        ∀ ⦃B₁ B₂ : Set StandardMinkowskiSpacetime.Carrier⦄
          ...
          Spacetime.IsCompletelySpacelike StandardMinkowskiSpacetime
            standardMinkowskiTimeOrientation B₁ B₂ →
          ∀ (a : N.U.algebra B₁) (b : N.U.algebra B₂),
            Commute (π (N.quasilocal.ι hB₁ a)) (π (N.quasilocal.ι hB₂ b)) := by
