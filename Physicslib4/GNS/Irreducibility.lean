...
private theorem smul_one_mem_centralizer (c : ℂ) (S : Set (H →L[ℂ] H)) :
    (c • 1 : H →L[ℂ] H) ∈ Set.centralizer S :=
  Set.mem_centralizer_iff.mpr fun M _ => by
...
scalars are central. -/
theorem center_gnsVonNeumann_eq_of_isIrreducible {π : A →⋆ₐ[ℂ] (H →L[ℂ] H)}
    (hirr : IsIrreducible π) :
...
`star (π a) = π (star a)`. -/
theorem range_selfAdjoint (π : A →⋆ₐ[ℂ] (H →L[ℂ] H)) :
    ∀ x ∈ Set.range π, star x ∈ Set.range π := by
  rintro x ⟨a, rfl⟩
...
noncomputable def gnsVonNeumannAlgebra (π : A →⋆ₐ[ℂ] (H →L[ℂ] H)) : VonNeumannAlgebra H :=
  vonNeumannOfSelfAdjoint (Set.range π) (range_selfAdjoint π)

@[simp] theorem coe_gnsVonNeumannAlgebra (π : A →⋆ₐ[ℂ] (H →L[ℂ] H)) :
    (gnsVonNeumannAlgebra π : Set (H →L[ℂ] H)) = gnsVonNeumann π :=
...
theorem coe_gnsVonNeumannAlgebra_eq_univ_of_isIrreducible {π : A →⋆ₐ[ℂ] (H →L[ℂ] H)}
    (hirr : IsIrreducible π) :
    (gnsVonNeumannAlgebra π : Set (H →L[ℂ] H)) = Set.univ := by
  rw [coe_gnsVonNeumannAlgebra]
  exact gnsVonNeumann_eq_univ_of_isIrreducible hirr
...
theorem gnsVonNeumannAlgebra_isGreatest_of_isIrreducible {π : A →⋆ₐ[ℂ] (H →L[ℂ] H)}
    (hirr : IsIrreducible π) (S : VonNeumannAlgebra H) :
