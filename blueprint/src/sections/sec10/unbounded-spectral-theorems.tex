...
\begin{definition}[The Product Hilbert Space $\mathbf{H} \times \mathbf{H}$]
    ...
    The one consequence to keep in mind is for later additions rather than for anything above. Nothing in this section uses orthogonality in $\mathbf{H}\times\mathbf{H}$: the adjoint's graph is shown closed directly, and closability follows from the observation that a subset of a graph is again a graph, so the familiar arguments through $\Gamma(A^*) = \big( V\Gamma(A) \big)^\perp$, with $V(\phi,\psi) = (-\psi,\phi)$, are never invoked. Any result added later that does need an orthogonal complement, an orthogonal decomposition of $\mathbf{H}\times\mathbf{H}$, or the self-adjointness of $A^*A$ must be stated over the $L^2$ product rather than the plain one. A reader comparing the two texts should accordingly not read the absence of the $L^2$ inner product from the graph declarations as an omission.
\end{definition}
...
\begin{theorem}[Dominated Convergence Theorem]
    ...
    \lean{MeasureTheory.tendsto_integral_of_dominated_convergence, MeasureTheory.Integrable.mono', aestronglyMeasurable_of_tendsto_ae}
    \uses{conv:section-integration}
    \leanok
...
\begin{lemma}[A Closed Subspace is a Separable Hilbert Space]
    ...
    \lean{IsClosed.completeSpace_coe, Submodule.innerProductSpace, Submodule.coe_norm, TopologicalSpace.IsSeparable.of_separableSpace, TopologicalSpace.IsSeparable.separableSpace}
    \leanok
...
\begin{theorem}[Norm via Dual Pairing]
    ...
    \lean{Physicslib4.Spectral.Unbounded.norm_eq_sSup_dual}
    \leanok
...
\begin{lemma}[Polynomials Preserve Almost Eigenvectors]
    ...
    \lean{Physicslib4.Spectral.Unbounded.exists_const_isAlmostEigenvector_mvApply}
    \leanok
