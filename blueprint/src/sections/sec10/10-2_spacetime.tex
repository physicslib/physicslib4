...
\begin{lemma}[Convexity of the Future Cone]
    ...
    \lean{Physicslib4.Spacetime.isFuturePointing_add}
    \leanfile{Physicslib4/Spacetime/LorentzOrthogonal.lean}
    \uses{def:future-and-past-pointing-vectors, lmm:cone-sign-lemma, lmm:timelike-cone-convexity}
    \leanok
    The sum of two timelike future-pointing tangent vectors (with respect to a fixed time orientation) is again timelike and future-pointing. Thus the timelike future-pointing vectors form a convex cone.
\end{lemma}
\begin{proof}
    \uses{lmm:cone-sign-lemma, lmm:timelike-cone-convexity}
    Let $v, w$ be timelike and future-pointing, so $g(t,v) < 0$ and $g(t,w) < 0$. By the sign lemma (\cref{lmm:cone-sign-lemma}) $g(v,w) < 0$, so $v$ and $w$ are aligned and $v + w$ is timelike by cone convexity (\cref{lmm:timelike-cone-convexity}). Moreover $g(t, v+w) = g(t,v) + g(t,w) < 0$ by bilinearity, so $v + w$ is future-pointing.
\end{proof}
