...
\begin{theorem}[An Irreducible Representation Generates $\mathcal{B}(H)$]
    \label{thrm:irreducible-generates-all}
    \lean{Physicslib4.GNS.gnsVonNeumann_eq_univ_of_isIrreducible}
    \leanfile{Physicslib4/GNS/Irreducibility.lean}
    \leanok
    \uses{def:irreducible-representation, def:local-von-neumann}
    The von Neumann algebra $\pi(A)''$ generated by an irreducible representation is \emph{all} of $\mathcal{B}(H)$: $\pi(A)'' = \mathcal{B}(H)$. By irreducibility the commutant $\pi(A)'$ is exactly the scalars, and the centralizer of the scalars is everything (every bounded operator commutes with $c \cdot 1$), so the bicommutant $\pi(A)'' = (\text{scalars})' = \mathcal{B}(H)$. This is the density (bicommutant-theorem) form of irreducibility, sharpening the factor statement \ref{thrm:irreducible-factor}.
\end{theorem}
