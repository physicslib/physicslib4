...
The classical theorem states that $\omega$ is pure if and only if its GNS representation is irreducible. The bridge \ref{thrm:commutant-scalar-iff} reduces the equivalence to the correspondence between dominated positive functionals and positive operators in the commutant (the GNS Radon-Nikodym theorem): a dominated $\psi$ arises as $\psi(a) = \langle \Omega, T\,\pi(a)\Omega\rangle$ for a unique $0 \le T \le 1$ in the commutant. Given that correspondence, $T$ scalar (irreducibility) is equivalent to $\psi$ proportional to $\omega$ (purity). The correspondence itself is a separate analytic development.
