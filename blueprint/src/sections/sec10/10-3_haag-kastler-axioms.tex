...
\begin{theorem}[Self-Inclusion Irreducible iff Factor]
    \label{thrm:irreducible-inclusion-self-factor}
    \lean{Physicslib4.AQFT.HaagKastler.HaagKastlerNet.isIrreducibleInclusion_self_iff_isFactor}
    \leanfile{Physicslib4/AQFT/HaagKastler/LocalVonNeumann.lean}
    \uses{def:irreducible-inclusion, def:local-von-neumann-algebra}
    For a single region $\mathbf{B}$, the self-inclusion $R(\mathbf{B}) \subseteq R(\mathbf{B})$ is irreducible if and only if $R(\mathbf{B})$ is a factor. The relative commutant of the self-inclusion is exactly the center $R(\mathbf{B}) \cap R(\mathbf{B})'$, so its triviality (irreducibility of the inclusion) coincides with triviality of the center (the factor property), up to commuting the intersection. This is the bidirectional companion to \ref{thrm:irreducible-inclusion-factor}.
\end{theorem}
