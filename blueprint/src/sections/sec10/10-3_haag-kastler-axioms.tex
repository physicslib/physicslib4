...
\begin{theorem}[KMS States are Invariant]
    ...
    \leanfile{Physicslib4/AQFT/KMS.lean}
    \leanok
    \uses{def:kms-state, thrm:strip-periodic-extension}
    At positive inverse temperature $\beta > 0$, every $(\alpha, \beta)$-KMS state $\omega$ is $\alpha$-invariant: $\omega(\alpha_t a) = \omega(a)$ for all $t$ and $a$. Taking $a = 1$, the KMS correlation function of $(1, a)$ has \emph{equal} boundary values $F(t) = F(t + i\beta) = \omega(\alpha_t a)$; by the strip Schwarz reflection (\ref{thrm:strip-periodic-extension}) it extends to a bounded entire function, hence is constant on $\mathbb{R}$ by Liouville, so $\omega(\alpha_t a) = F(t) = F(0) = \omega(a)$. The positivity $\beta > 0$ is essential: at $\beta \le 0$ the strip degenerates and the conclusion fails.
\end{theorem}
