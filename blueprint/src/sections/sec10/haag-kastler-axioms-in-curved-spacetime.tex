...
\begin{theorem}[Geometric Covariance of the von Neumann Net (Curved Spacetime)]
    ...
    \lean{Physicslib4.AQFT.HaagKastlerCurved.HaagKastlerNet.lieConj_image_localVonNeumann, Physicslib4.AQFT.HaagKastlerCurved.HaagKastlerNet.lieConj_image_localOperators, Physicslib4.AQFT.HaagKastlerCurved.HaagKastlerNet.stabAutHom_commIsotony}
    ...
    Unlike Minkowski, the abstract \texttt{LorentzianSpacetime} interface does not supply basis-set preservation $M.\mathrm{IsBasisSet}(g \cdot \mathbf{B}_1)$, so it enters as an explicit hypothesis, discharged for a net from a concrete geometric spacetime. The coherence relating the stabilizer action $\hat\alpha_g$ to the isotony embeddings of Axiom 2 (\ref{def:isotony-in-curved-spacetime}) is not a hypothesis: it is condition (3) of Axiom 5 (\ref{def:isometric-covariance-in-curved-spacetime}), transported along $g \cdot \mathbf{B} = \mathbf{B}$.
\end{theorem}
\begin{proof}
    \leanok
