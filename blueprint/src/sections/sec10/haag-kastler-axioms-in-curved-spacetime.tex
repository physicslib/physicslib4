...
\begin{theorem}[Geometric Covariance of the von Neumann Net (Curved Spacetime)]
    ...
    Unlike Minkowski, the abstract \texttt{LorentzianSpacetime} interface does not supply basis-set preservation $M.\mathrm{IsBasisSet}(g \cdot \mathbf{B}_1)$, so it enters as an explicit hypothesis. For a net over a concrete Lorentzian spacetime, with the isometries connected to the identity, it is discharged automatically (\texttt{lieConj\_image\_localVonNeumann\_identityComponent}). The inclusion $g \cdot \mathbf{B}_1 \subseteq \mathbf{B}$ is not a hypothesis: it follows from $\mathbf{B}_1 \subseteq \mathbf{B}$ and $g \cdot \mathbf{B} = \mathbf{B}$. The coherence relating the stabilizer action $\hat\alpha_g$ to the isotony embeddings of Axiom 2 (\ref{def:isotony-in-curved-spacetime}) is not a hypothesis: it is condition (3) of Axiom 5 (\ref{def:isometric-covariance-in-curved-spacetime}), transported along $g \cdot \mathbf{B} = \mathbf{B}$.
\end{theorem}
\begin{proof}
    \leanok
    \uses{def:local-von-neumann-in-curved-spacetime, thrm:gns-unitary-stabilizer-in-curved-spacetime, thrm:von-neumann-geometric-covariance, def:isometric-covariance-in-curved-spacetime}
