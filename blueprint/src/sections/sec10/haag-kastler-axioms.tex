...
\begin{definition}[Axiom 3: Local Commutativity]
    ...
    By \ref{lmm:local-commutativity-any-quasilocal} the choice of quasilocal algebra in this axiom is immaterial, so the canonical $\mathfrak{U}$ of a Haag--Kastler net can be used throughout.
\end{definition}
...
\begin{lemma}[Local Commutativity Holds in Every Quasilocal Algebra]
    ...
    \leanok
    Suppose the net satisfies Axiom 3 (\ref{def:local-commutativity}) with respect to some quasilocal algebra $\mathfrak{U}$, i.e. an ambient C*-algebra with injective unital $*$-homomorphisms $\iota_{\mathbf{B}} : \mathfrak{U}(\mathbf{B}) \to \mathfrak{U}$ satisfying the cocone condition $\iota_{\mathbf{B}_2} \circ i_{\mathbf{B}_1\mathbf{B}_2} = \iota_{\mathbf{B}_1}$ for all basis sets $\mathbf{B}_1 \subseteq \mathbf{B}_2$. Let $\mathfrak{U}'$ be any quasilocal algebra of the net (\ref{def:quasilocal-algebra}), with canonical embeddings $\iota'_{\mathbf{B}}$. Then for any completely spacelike basis sets $\mathbf{B}_1$, $\mathbf{B}_2$ and any $a_1 \in \mathfrak{U}(\mathbf{B}_1)$, $a_2 \in \mathfrak{U}(\mathbf{B}_2)$,
    \begin{align}
        \iota'_{\mathbf{B}_1}(a_1)\, \iota'_{\mathbf{B}_2}(a_2) = \iota'_{\mathbf{B}_2}(a_2)\, \iota'_{\mathbf{B}_1}(a_1)
    \end{align}
    in $\mathfrak{U}'$. In particular Axiom 3 holds in the canonical quasilocal algebra of \ref{thrm:quasilocal-algebra-exists}, and does not depend on which quasilocal algebra is used to state it.
\end{lemma}
\begin{proof}
    \leanok
    \uses{def:local-commutativity, def:isotony, lmm:minkowski-diamonds-upward-directed}
    By upward directedness of the Alexandrov diamonds (\ref{lmm:minkowski-diamonds-upward-directed}) pick a basis set $\mathbf{B}$ with $\mathbf{B}_1 \cup \mathbf{B}_2 \subseteq \mathbf{B}$. By the cocone condition, in any quasilocal algebra $\iota_{\mathbf{B}_k}(a_k) = \iota_{\mathbf{B}}(i_{\mathbf{B}_k\mathbf{B}}(a_k))$ for $k = 1, 2$, where $i_{\mathbf{B}_k\mathbf{B}}$ are the isotony embeddings of Axiom 2 (\ref{def:isotony}). Since $\iota_{\mathbf{B}}$ is an injective $*$-homomorphism, $\iota_{\mathbf{B}_1}(a_1)$ and $\iota_{\mathbf{B}_2}(a_2)$ commute if and only if $i_{\mathbf{B}_1\mathbf{B}}(a_1)$ and $i_{\mathbf{B}_2\mathbf{B}}(a_2)$ commute in $\mathfrak{U}(\mathbf{B})$. That condition does not mention the quasilocal algebra, so commutation in the algebra for which Axiom 3 is assumed transfers to $\mathfrak{U}'$ with its embeddings $\iota'_{\mathbf{B}}$.
\end{proof}
...
\begin{theorem}[Geometric Covariance of the von Neumann Net]
    ...
    \lean{Physicslib4.AQFT.HaagKastler.HaagKastlerNet.lieConj_image_covLocalVonNeumann, Physicslib4.AQFT.HaagKastler.HaagKastlerNet.lieConj_image_covLocalOperators}
...
\begin{theorem}[Orbit-Invariance of Factoriality]
    ...
    \lean{Physicslib4.AQFT.HaagKastler.HaagKastlerNet.covLocalVonNeumann_isFactor_smul}
    \leanfile{Physicslib4/AQFT/HaagKastler/GeometricCovariance.lean}
...
\begin{theorem}[Geometric Covariance as a von Neumann Algebra Isomorphism]
    ...
    \lean{Physicslib4.AQFT.HaagKastler.HaagKastlerNet.covLocalVonNeumannEquiv}
    \leanfile{Physicslib4/AQFT/HaagKastler/GeometricCovariance.lean}
    \uses{def:local-von-neumann-algebra}
    \leanok
...
\begin{theorem}[Existence of the Quasilocal Lift]
    ...
    \leanok
    For every quasilocal algebra of a Haag--Kastler net, the fiberwise Lorentz action extends to a $*$-automorphism of $\mathfrak{U}$, so the lift of \ref{def:quasilocal-lift} exists. No covariance-compatibility hypothesis is needed: by \ref{lmm:quasilocal-covariance-compatible} it holds for every quasilocal algebra.
\end{theorem}
\begin{proof}
    \leanok
...
\begin{theorem}[GNS Unitary Implementation of an Invariant State]
    ...
    \lean{Physicslib4.AQFT.HaagKastler.HaagKastlerNet.IsInvariantState.exists_gns_unitary}
    \leanfile{Physicslib4/AQFT/HaagKastler/QuasilocalIntertwiner.lean}
...
\begin{theorem}[Purity is Covariance-Invariant]
    ...
    \lean{Physicslib4.GNS.isPure_precomp_iff, Physicslib4.AQFT.HaagKastler.HaagKastlerNet.isPure_precomp_action_iff}
    \leanfile{Physicslib4/GNS/ExtremeState.lean}
